...
for parent,children in sorted(mid2children.items(), key=lambda x: count_subs(x[0])):
    print(count_subs(parent),mid2label[parent] if parent in mid2label else parent,[(mid2label[child[0]], child[1], child[2]) for child in children if child[1]=='Subcategory'])