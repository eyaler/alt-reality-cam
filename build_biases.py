...
threshold = 0.4
sim = 'manual' #'pairdirection', '3cosadd', '3cosmul', '2cosadd', '2cosmul', 'manual'

biases = [('zero',), ('gender','woman','man'), ('military','war','peace'), ('money','rich','poor'), ('love','love','hate')]
...
def pair_direction(positive, negative, topn=None):
    vocab = [word for word in word_vectors.vocab if word not in positive+negative]
...
def get_bias(label, bias, sim):
    ...
    if not sim.startswith('2') and len(bias)>1 and bias[0] in labels and np.allclose(word_vectors[label], word_vectors[bias[1]]):
        ...
        biased = func(positive=[label, bias[0]], negative=None if sim.startswith('2') else [bias[1]], topn=1+len(shifts))
        biased = [label[0] for label in biased if label[0] not in shifts][0]
    return biased
