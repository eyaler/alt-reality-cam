...
with open('image_urls.txt') as fin:
    for image_url in fin:
        ...
        _, result["detection_scores"], result["detection_boxes"], result["detection_class_names"] = zip(*[x for x in sorted(zip(detection_cnt_subs, result["detection_scores"], result["detection_boxes"], result["detection_class_names"]), key=lambda x: (x[0],-x[1],x[2],x[3])) if x[1]>supression_threshold][:supression_topk])
        ...
        sim = find_similar(result["detection_boxes"], result["detection_class_names"], top_k=images_topk, hierarchy_factor=hierarchy_factor, polypedo_discount=polypedo_discount)
        scores = []
        ...
        found = [[mid2label[l] for l in filter_duplicate_boxes(s[3], s[4])] for s in sim]
        ...
        for json_folder in [serve_path, folder]:
            with open(os.path.join(json_folder, 'labels.json'), 'w') as fout:
                json.dump(json_dict, fout, ident='\t')
